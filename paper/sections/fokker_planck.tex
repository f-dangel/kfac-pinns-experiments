...
We consider the logarithmic Fokker-Planck equation from \Cref{eq:log-fokker-planck-isotropic-gaussian}.
...
\Cref{fig:log_fokker_planck_10d-appendix} visualizes the results:
...
\begin{figure}[!h]
  ...
  \end{subfigure}
  \caption{Training loss and evaluation $L_2$ error for learning the solution to a 9+1d logarithmic Fokker-Planck equation over (\subref{subfig:log_fokker_planck_10d-time}) time and (\subref{subfig:log_fokker_planck_10d-step}) steps.}\label{fig:log_fokker_planck_10d-appendix}
\end{figure}

\paragraph{Best run details} The runs shown in \Cref{fig:log_fokker_planck_10d-appendix} correspond to the following hyper-parameters:
...
\paragraph{Search space details} The runs shown in \Cref{fig:log_fokker_planck_10d-appendix} were determined to be the best via a random search on the following search spaces which each optimizer given approximately the same total computational time ($\mathcal{U}$ denotes a uniform, and $\mathcal{LU}$ a log-uniform distribution):
\begin{itemize}
  \def\pathToRuns{kfac_pinns_exp/exp43_log_fokker_planck9d_isotropic_gaussian_random/tex}
