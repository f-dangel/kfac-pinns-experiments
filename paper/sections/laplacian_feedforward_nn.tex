...
\begin{figure}[t]
  ...
  \caption{Computation graph of a sequential neural network's Laplacian $\Delta u$ with $u \in \sR$.
    ...
    Note that $\vz^{(0)}\coloneqq \vx$, $\vz^{(L)} \coloneqq u$, $\grad_uu = 1$, and $\gradsquared{u}u = \vzero$.
...
Some insights while creating \Cref{fig:hbp-dependencies}:
\begin{itemize}
\item If the last layer is a linear layer, it's weight does not contribute to the Hessian during the Hessian backward pass.
  This is because the incoming Hessian is $\vzero$, and the Hessian backpropagation is $\mW^T\vzero \mW = \vzero$.
  \toodoo{Does something similar apply for the contribution from the gradient backpropagation?}

\item About the bias terms.
  I think they only contribute through the forward pass, but not otherwise as they do not show up in any of the operations used during gradient/Hessian backpropagation.
\end{itemize}
