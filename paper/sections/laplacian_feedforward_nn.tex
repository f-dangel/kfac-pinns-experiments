...
However, we can simplify into 9 terms, as the Jacobian from \Cref{subeq:fisher-jacobians-hbp} uses a symmetric $\mX$ (the Hessian w.r.t.\,the layer's output).
So we have
\begin{align*}
...
\toodoo{Visualize the different contributions on a $3\times 3$ grid.}
...
\toodoo{F.D.\,Write down the full expression for the Fisher.
  Propose ways to Kronecker-approximate each of the 9 terms, such that we end up with a single Kronecker product in the end.}
