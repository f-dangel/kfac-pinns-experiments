...
\item If the last layer is a linear layer, it's weight does not contribute to the Hessian during the Hessian backward pass.
  ...
  \begin{align*}
    ...
    &=
      ...
      \vzero
      \shortintertext{and}
      \mF^{(L)}_{\bullet, \gradsquared{\vz^{(L-1)}}u}
    &=
      \vzero
      =
      \mF^{(L)}_{\gradsquared{\vz^{(L-1)}}u, \bullet}
      \qquad \forall \bullet\,.
  \end{align*}
  We can see this effect clearly in the (forward, Hessian) and (Hessian, Hessian) panels of \Cref{fig:gramian-contribution-children} where any row/column associated to the last layer's weight is zero.
  \toodoo{F.D.
    Surprisingly, this does not seem to hold for the (gradient, Hessian) panel.
    Could be numerical artifacts because overall the Gramian's elements are pretty small, or a bug in my code.}
...
\item So far we only discussed weights.
  ...
  We should see that in all panels except for (forward, forward) of \Cref{fig:gramian-contribution-children}.
  \toodoo{However, currently only the (gradient, gradient), (gradient, Hessian), and (Hessian, Hessian) panels are consistent with this.
    Need to debug why there is an expectation mismatch for (forward, gradient) and (forward, Hessian).}
\end{itemize}
