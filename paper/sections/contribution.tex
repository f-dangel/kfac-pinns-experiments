...
%\begin{align}
       ...
       %\partial_{x_i} \partial_{x_j} (f \circ g)(\vx) & = \partial_{x_i} g(\vx)^\top \nabla^2f(g(\vx))\partial_{x_j} g(\vx) + \jac f(g(\vx)) \partial_{x_i} \partial_{x_j} g(\vx).
%\end{align}
...
%        \Delta \vz^{(l)} & = \sum_i \sigma''(\vz^{(l-1)}) \odot (\partial_{x_i} \vz^{(l-1)})^{\odot 2} + \sigma'(\vz^{(l-1)}) \odot \Delta \vz^{(l-1)}
%    \end{align}
%\end{subequations}
\begin{comment}
    \begin{align}
