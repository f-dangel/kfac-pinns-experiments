...
\end{equation}
where $D^m_\vx% = (\partial_\vx^\alpha u)_{\lvert \alpha \rvert = l}
$ collects all partial derivatives of order $m$.
...
the linear layer's weight is shared over the matrices $D^{0}_\vx \vz^{(l-1)} \coloneqq \vz^{(l-1)}, D_\vx^1 \vz^{(l-1)}, \dots, D_\vx^k \vz^{(l-1)}$. Stacking them into a matrix $\mZ^{(l-1)}_n = (\vz^{(l-1)}, D_\vx^1 \vz^{(l-1)}, \dots, D_\vx^k \vz^{(l-1)} ) \in \sR^{h^{(l-1)} \times S}$, we can use the chain rule to obtain
\begin{align}
  \begin{split}
    \mG^{(l)}_{\Omega}(\vtheta)
    &=
...
\textbf{Gradient pre-conditioning}
Given layer $l$'s mini-batch gradient $\vg_t^{(l)} = \nicefrac{\partial L(\vtheta_t)}{\partial\vtheta^{(l)}_t} \in \sR^{p^{(l)}}$, we obtain an update direction $\vDelta_t^{(l)} = -(\tilde{\mA}^{(l)}_{\Omega,t} \otimes \tilde{\mB}^{(l)}_{\Omega,t} + \tilde{\mA}^{(l)}_{\partial\Omega} \otimes \tilde{\mB}^{(l)}_{\partial\Omega})^{-1} \vg_t^{(l)}$.
We use the trick of \cite[Appendix I]{martens2015optimizing} to invert the Kronecker sum which requires eigen-decomposing all Kronecker factors.

\textbf{Learning rate and momentum} From the pre-conditioned gradient $\vDelta_t \in \sR^P$, we consider two different updates $ \vtheta_{t+1} = \vtheta_t + \vdelta_{t}$ we call `KFAC' and `KFAC*'.
...
It parameterizes the iteration's update as $\vdelta_{t+1}(\alpha, \mu) = \alpha \vDelta_t + \mu \vdelta_t$, then obtains the optimal parameters by minimizing the quadratic model $m(\vdelta_{t+1}) = L(\vtheta_t) + \vdelta_{t+1}^{\top} \vg_t + \nicefrac{1}{2}\vdelta_{t+1}^{\top} (\mG(\vtheta_t) + \lambda \mI) \vdelta_{t+1}$ with the exact damped Gramian.
The optimal learning rate and momentum $\argmin_{\alpha, \mu} m(\vdelta_t)$ are
\begin{align*}
  \begin{pmatrix}
  ...
  -
  \begin{pmatrix}
    \vDelta_t^{\top} \mG_t \vDelta_t + \lambda \left\lVert \vDelta_t \right\rVert^2
    & \vDelta_t^{\top} \mG_t \vdelta_t + \lambda \vDelta^{\top}_t \vdelta_t
    \\
    \vDelta_t^{\top} \mG_t \vdelta_t + \lambda \vDelta^{\top}_t \vdelta_t
    &
      \vdelta_t^{\top} \mG_t \vDelta_t + \lambda \left\lVert \vdelta_t \right\rVert^2
...
The computational cost is dominated by the two Gramian-vector products $\mG_t \vDelta_t$ and $\mG_t \vdelta_t$.
By using the Gramian's outer product structure~\cite{dangel2022vivit,papyan2019measurements}, we perform them with automatic differentiation~\citep{pearlmutter1994fast,schraudolph2002fast} using two Jacobian-vector products, as recommended in \cite{martens2015optimizing}.
