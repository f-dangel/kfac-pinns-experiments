...
%a popular choice as a Gramian is given by the Gauss-Newton matrix
%\begin{equation}
...
classic Gauss-Newton matrix~\citep{martens2020new, eschenhagen2023kroneckerfactored}.
%Here, we have used the suggestive short-hand notation $\vu_n = u_\vtheta(\vx_n)$.
To distinguish between the Gauss-Newton matrix $\mG(\vtheta)$ for PINNs, we refer to the Gauss-Newton matrix $\mF(\vtheta)$ as the Fisher matrix or Fisher.
...
The parameters $\vtheta^{(l)} = \mW^{(l)}$ of the $l$-th layer appears in the computational graph by $\vz^{(l)} = \mW^{(l)}\vz^{(l-1)}$ and %by the vec-trick,
...
%\end{align}
Summing over the data points and using  $\sum_n \mA_n \otimes \mB_n \approx N^{-1}(\sum_n \mA_n) \otimes (\sum_n \mB_n)$ %~\citep{martens2015optimizing}
 we obtain
\begin{equation}
